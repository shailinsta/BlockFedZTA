from ci_utils import print_ci_results, save_ci_to_csv
import os
import numpy as np
import pandas as pd
import warnings
...
from scipy.stats import ttest_rel, t   # ✅ added t
...
# =========================
# ✅ CONFIDENCE INTERVAL FUNCTION (ADDED)
# =========================
...
def run_experiment(name, cfg, NUM_CLIENTS=8):
    ...
    print("\nFINAL:")
    print(f"Trust Mean: {np.mean(accs):.4f}")
    print(f"FedAvg Mean: {np.mean(fedavg_accs):.4f}")
...
for s in RESULTS:
    t_mean = np.mean(RESULTS[s]["trust"])
    f_mean = np.mean(RESULTS[s]["fed"])
    print(f"{s:<15} {t_mean:.4f}   {f_mean:.4f}")

print("="*60)


# =========================
# ✅ 95% CONFIDENCE INTERVAL (ADDED ONLY)
# =========================
print("\n" + "="*60)
...
# =========================
print("\n" + "="*70)
print(f"{'SCENARIO':<15} {'METHOD':<12} {'ACCURACY (mean ± std)':<30}")
print("="*70)
...
for s in RESULTS:
    ...
    t_mean, t_std = trust.mean(), trust.std()
    f_mean, f_std = fed.mean(), fed.std()
    ...
    print(f"{s:<15} {'Trust':<12} {t_mean:.4f} ± {t_std:.4f}")
    print(f"{'':<15} {'FedAvg':<12} {f_mean:.4f} ± {f_std:.4f}")
